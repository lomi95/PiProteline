...
test_that("unweighted_network_analysis performs basic analysis", {
  ...
  result <- unweighted_network_analysis(data.grouped = data.grouped, names_of_groups = c("Group1", "Group2"),
  ...
  expect_true(all(c("PPI_unweighted", "Centralities", "CriticalNodes") %in% names(result)))
})

test_that("unweighted_network_analysis handles data.unique when data.grouped is NULL", {
  ...
  result <- unweighted_network_analysis(data.grouped = NULL, names_of_groups = c("Group1", "Group2"), data.unique = data.unique,
                                        fun_list = list(Degree = igraph::degree, Betweenness = igraph::betweenness),
                                        g.interactome = g.interactome,
                                        quantile_critical_nodes = 0.8)
...
test_that("unweighted_network_analysis raises an error when both data.grouped and data.unique are NULL", {
  expect_error(unweighted_network_analysis(data.grouped = NULL, names_of_groups = c("Group1", "Group2")),
               "Either 'data.unique or 'data.grouped' must be given")
})
