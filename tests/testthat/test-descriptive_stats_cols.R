########################################test1########################################
test_that("descriptive_stats_cols works with numeric data", {
  # Create test data
  ...
  # Apply the function
  result <- suppressMessages(descriptive_stats_cols(test_data))
...
########################################test2########################################
test_that("descriptive_stats_cols handles columns with only zeros", {
  zero_data <- data.frame(
    col1 = c(0, 0, 0),
    col2 = c(0, 0, 0)
  )

  result <- suppressMessages(descriptive_stats_cols(zero_data))
...
########################################test3########################################
test_that("descriptive_stats_cols handles columns with missing values", {
  missing_data <- data.frame(
    col1 = c(1, 2, NA, 4, 5),
    col2 = c(10, NA, 0, 40, 50)
  )

  result <- suppressMessages(descriptive_stats_cols(missing_data))
...
########################################test4########################################
test_that("descriptive_stats_cols handles data frames with no numeric columns", {
  no_numeric_data <- data.frame(
    col1 = c("a", "b", "c"),
    col2 = c("x", "y", "z")
  )

  result <- suppressMessages(descriptive_stats_cols(no_numeric_data))

  # Since there are no numeric columns, the result should be an empty data frame
  expect_equal(nrow(result), 0)
})
########################################test5########################################
test_that("descriptive_stats_cols handles data frames with mixed types", {
  ...
  result <- suppressMessages(descriptive_stats_cols(mixed_data))
...
########################################test6########################################
test_that("descriptive_stats_cols throws an error for non-data-frame input", {
  expect_error(descriptive_stats_cols(123), "Input must be a data frame.")
  expect_error(descriptive_stats_cols("test"), "Input must be a data frame.")
  expect_error(descriptive_stats_cols(list(a = 1, b = 2)), "Input must be a data frame.")
})
########################################test7########################################
test_that("descriptive_stats_cols handles empty data frames", {
  empty_data <- data.frame()
  expect_warning(descriptive_stats_cols(empty_data),"Input data frame is empty. No statistics to compute.")
})
