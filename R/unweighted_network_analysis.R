...
#'                                                                "database"    = 0.35))
#' g.interactome <- graph_from_edgelist(as.matrix(filteredInteractome[,3:4]),
#'                                      directed = FALSE)
#' set.seed(1)
...
#'   rownames(x) <- sample(names(V(g.interactome)), nrow(x))
#'   return(x)
#' })
#' colnames(data.grouped$Group1) <- paste0("Group1_",1:10)
#' colnames(data.grouped$Group2) <- paste0("Group2_",1:10)
#' result <- unweighted_network_analysis(data.grouped = data.grouped,
...
#'
#'
unweighted_network_analysis <- function(data.grouped = NULL, names_of_groups, data.unique = NULL,
  ...
  if (is.null(data.grouped)){
    if (is.null(data.unique)){
      stop("Either 'data.unique or 'data.grouped' must be given")
    }
  ...
  PPI_unweighted <- lapply(data.grouped, function(x){
    G <- rownames(x)
    if (length(intersect(names(igraph::V(g.interactome)),G)) == 0){
      stop("No gene names were found in the interactome given")
    }
    igraph::induced_subgraph(g.interactome, intersect(names(igraph::V(g.interactome)),G))
  })
  ...
  CN.unweighted <- lapply(search_mode, function(x){

    Hubs <- criticalNodes(Unweighted.q, names_of_groups,
                          centralities = c("Betweenness","Centroids"),
                          searching.mode = x)
    Bottlenecks <- criticalNodes(Unweighted.q, names_of_groups,
                                 centralities = c("Betweenness","Bridging"),
                                 searching.mode = x)
    HB <- criticalNodes(Unweighted.q, names_of_groups,
                        centralities = c("Betweenness","Centroids","Bridging"),
                        searching.mode = x)
    return(list(Hubs = Hubs,
                Bottlenecks = Bottlenecks,
                HubsAndBottlenecks = HB))
  })


  return(list(PPI_unweighted = PPI_unweighted,
              Centralities = Unweighted,
              CriticalNodes = CN.unweighted))
}
