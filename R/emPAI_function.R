...
#'
#' @param list.groups_peptides A list where each element represents a group of peptides (e.g., numeric vectors or matrices of peptide data), or a dataset where rows are samples and columns are proteins.
#' @param Pn_ideal A numeric vector representing the ideal number of peptides for each protein.
#'
...
#' where \code{Obs} is the observed number of peptides, and \code{Pn_ideal} is the ideal number of peptides for each protein.
#'
...
#' emPAI_function(list.groups_peptides, Pn_ideal)
#'
#' # Using a dataset (Sample x Protein)
#' dataset <- matrix(c(5, 10, 15, 8, 12, 20), nrow = 2)
#' emPAI_function(dataset, Pn_ideal)
#'
#' @export
