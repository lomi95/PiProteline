...
#' treeplot(spe_result$Process, "Process", first_n = 20)
#' }
#'

treeplot <- function(category_spe, title_plot,
                     first_n = 30,
  ...
  treePlot <- ggtree(newick, branch.length = "none", layout = "circular", open.angle = 15)  +
    ...
    xlim(0, 20) +
    geom_tiplab(size = 3.5, offset = 2*length(av_columns))

  for (i in seq_along(av_columns)) {
    group <- av_columns[i]
    group_data <- single_spe_filter2[,
                                     c(1,2,3, ncol(single_spe_filter2), grep(group, colnames(single_spe_filter2)))]
    colnames(group_data)[5] <- "val_group"
    group_data$group <- factor(group,levels = av_columns)

    if (nrow(group_data) > 0) {
      treePlot <- treePlot +
        geom_fruit(
          data = group_data,
          mapping = aes(y = description_wrapped, x = val_group, fill = group),
          geom = geom_col
        )
    }
  }
