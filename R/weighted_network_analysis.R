...
#'
#' @param data.grouped.evenDim A list of data frames representing grouped data, with same numbero of genes. If `NULL`, data is grouped using `data.unique` and the specified parameters.
...
#' @param g.interactome An igraph object representing the interactome network.
...
#'
#'
weighted_network_analysis <- function(data.grouped.evenDim = NULL, names_of_groups, data.unique = NULL,
                                      fun_list, g.interactome, quantile_critical_nodes, ...) {
  args_list <- list(...)

  search_mode <- c("","s","cs")
  names(search_mode) <- c("NotSpecific","Specific","CentralitySpecific")

  if (is.null(data.grouped.evenDim)) {
    if (is.null(data.unique)) {
      stop("Either 'data.unique' or 'data.grouped' must be provided")
    }
  ...
  args_Corr_PPI.model <- args_list[intersect(names(args_list), names(formals(Corr_PPI.model)))]
  PPI_correlations <- lapply(data.grouped.evenDim, function(x) {

    do.call(Corr_PPI.model, c(list(dataset = transpose(x, gene_column = 0),
                                   interactome.graph = g.interactome,
                                   args_Corr_PPI.model)))
  ...
  CN.weighted <- lapply(search_mode, function(x) {

    Hubs <- criticalNodes(Weighted.q, names_of_groups, centralities = c("Betweenness", "Centroids"),
                          searching.mode = x)
    Bottlenecks <- criticalNodes(Weighted.q, names_of_groups, centralities = c("Betweenness", "Bridging"),
                                 searching.mode = x)
    HB <- criticalNodes(Weighted.q, names_of_groups, centralities = c("Betweenness", "Centroids", "Bridging"),
                        searching.mode = x)

    list(Hubs = Hubs, Bottlenecks = Bottlenecks, HubsAndBottlenecks = HB)
  })

  return(list(PPI_correlations = PPI_correlations,
              Centralities = Weighted,
              CriticalNodes = CN.weighted))
}
